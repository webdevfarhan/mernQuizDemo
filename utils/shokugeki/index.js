...
const shokugekiData = require('./data');

const shokugeki = async (appname, filename, dp, myname, gender) => {
  const loadAll = async () => {
    const jimpImage = jimp.read(dp);
    let r;
    gender === 'female' ? (r = uniqueRandomArray(shokugekiData.charNamesFemales)) : (r = uniqueRandomArray(shokugekiData.charNamesMales));
  ...
  const jimpFunctions = async () => {
    resultImage.blit(jimpImage, 41, 182);
    resultImage.blit(watermark, 600, 350);
    resultImage.blit(charImage, 566, 182);
    resultImage.print(
      font,
      35,
      135,
